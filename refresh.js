...
module.exports = function start(refreshRate, config) {
    if (!refreshRate || !config) {
        return console.error(new TypeError("Missing parameters, either 'refreshRate' or 'config'"))
    }

    var rows = []
    var table = require("./controller").sessionConfig.table;
    table = table.sort((first, second) => first.row < second.row && first.col < second.col)
